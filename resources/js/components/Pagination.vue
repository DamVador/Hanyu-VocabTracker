...
<template>
    <div v-if="pagination && pagination.last_page >= 1" class="mt-6 mb-3 flex flex-col sm:flex-row justify-center items-center gap-4">
        <button
            type="button"
            ...
            :class="{ 'opacity-50 cursor-not-allowed': !pagination.prev_page_url || router.processing }"
        >
            &laquo; Previous
        </button>
        ...
        <div class="flex items-center gap-2">
            ...
            <template v-else>
                <button
                    v-for="page in simplePageButtons"
                    :key="page"
                    type="button"
                    @click.prevent.stop="goToPage(page)"
                    :class="[
                        'px-4 py-2 border rounded-md shadow-sm text-sm font-medium',
        ...
        <button
            ...
            class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
            :class="{ 'opacity-50 cursor-not-allowed': !pagination.next_page_url || router.processing }"
        >
            Next &raquo;
        </button>
    </div>
